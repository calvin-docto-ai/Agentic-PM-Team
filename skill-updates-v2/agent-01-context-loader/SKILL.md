...
**IMPORTANT — Use Lightweight Jira Extraction**: Jira tickets often return large JSON payloads (observed in Run 4: 25 tickets required Python extraction scripts to parse). To avoid this overhead, **always use the lightweight extraction protocol below**.

#### Lightweight Jira Extraction Protocol

When querying Jira via `searchJiraIssuesUsingJql`, request **only these fields**: `summary`, `status`, `key`, `priority`, `assignee`. Do NOT fetch full ticket bodies, comments, or attachments in the initial query.

**Step 3a — Run the JQL query**:
- Use JQL like: `project = "<PROJECT_KEY>" AND status in ("Open", "In Progress", "Done") AND updated >= -90d ORDER BY priority ASC, updated DESC`
- Request only the fields listed above

**Step 3b — Categorize and count** (do this in your synthesis, not via additional API calls):
- Count by type: bugs vs. features vs. tasks
- Count by priority: P0, P1, P2, P3+
- Count by status: Open, In Progress, Recently Closed
- Note total backlog size

**Step 3c — Flag notable tickets** (only fetch full details for these, if needed):
- P0/P1 bugs (active pain)
- Tickets with "blocked", "dependency", "risk" labels
- Tickets assigned to multiple people or reassigned frequently (indicates churn)
- Any ticket mentioned in Slack discussions (cross-reference from Step 2)

**What to extract per notable ticket** (if you need more detail):
- Key, summary, status, priority, assignee — that's it for the context document
- Do NOT paste raw JSON into the context output

**Output format for Jira section**:
```
### Jira Summary
- **Total tickets (last 90 days)**: 47 (32 bugs, 12 features, 3 tasks)
- **Priority distribution**: 2 P0, 5 P1, 18 P2, 22 P3+
- **Status**: 15 Open, 8 In Progress, 24 Recently Closed
- **Backlog trend**: Growing (+12 net new in last 30 days)

### Notable Tickets
| Key | Summary | Status | Priority |
|-----|---------|--------|----------|
| TLE-142 | Routing fails for multi-site specialists | Open | P0 |
| TLE-138 | Response time > 48h for 12% of cases | In Progress | P1 |
| ... | ... | ... | ... |
```

**What NOT to do**:
- Do NOT dump raw Jira JSON into the context document
- Do NOT fetch full ticket descriptions for all tickets — only for notable ones
- Do NOT write Python scripts to parse Jira output — the lightweight query should return clean, small payloads

**Legacy Jira search tips** (still useful for manual exploration):
...
For **same-day re-runs of the same product area**, you may reuse a previous Agent 1 context output instead of re-gathering from scratch. This pattern was validated in Run 4 (2026-03-22), where reusing Run 2's context (only 7 hours old) saved significant time without sacrificing freshness.

### When to Reuse

- The previous run was for the **same product area** (exact match)
- The previous run was on the **same calendar day** (less than ~16 hours old)
- The previous run's Agent 1 page still exists on Confluence and has content

### When NOT to Reuse

- Different product area (even a related one — always gather fresh)
- Previous run was on a different day (data may have shifted overnight)
- You have reason to believe significant new information has appeared (e.g., a major incident, a launch, a reorg)

### How to Reuse

1. Check for existing runs today using `getConfluencePageDescendants` on page `3870819267`
2. Find the most recent run for the same product area
3. Read its "Agent 1 — Context" child page using `getConfluencePage`
4. Copy the content into a **new** "Agent 1 — Context" child page for this run
5. Add a prominent reuse notice at the top of the page:

```markdown
> **⚡ Context Reused**: This context was originally gathered in Run [N] on [date] at approximately [time]. Freshness: [X] hours.
> For a different product area or next-day runs, fresh context is always gathered.
> Original source: [link to original Agent 1 page]
```

6. Do NOT silently reuse context — the notice must be visible so reviewers know they're looking at cached data
